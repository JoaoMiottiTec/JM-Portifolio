...
const Header: React.FC<HeaderProps> = ({ theme, setTheme }) => {
  ...
  const handleMenuClick = (setVisible: React.Dispatch<React.SetStateAction<boolean>>) => {
    setVisible(false);
  };
  ...
  return (
    <header ref={headerRef} className={`fixed top-0 w-full z-50 py-4 px-8`}>
      <div className="container mx-auto flex justify-center relative bg-transparent">
        <div className={`absolute inset-0 lg:w-5/6 rounded-full z-[-1] ${theme === 'light' ? 'bg-white text-black' : 'bg-black text-white'}`} style={{ width: '100%', margin: 'auto' }}></div>
        <div className="flex items-center justify-center space-x-4 rounded-full w-full lg:w-5/6 border p-4">
          ...
          >
            ...
            {navMenuVisible && (
              ...
              >
                <div className="py-1" onClick={() => handleMenuClick(setNavMenuVisible)}>
          ...
          >
            ...
            {socialMenuVisible && (
              <div
                ...
                className={`absolute mt-2 w-56 origin-top-left border border-gray-700 rounded-md shadow-lg focus:outline-none transition-transform transform ${theme === 'light' ? 'bg-white text-black' : 'bg-black text-white'}`}
              >
                <div className="py-1" onClick={() => handleMenuClick(setSocialMenuVisible)}>
                  <a
              ...
              >
                <div className="py-1">
                  <button
                    onClick={() => {
                      toggleTheme('light');
                      handleMenuClick(setThemeMenuVisible);
                    }}
